...
export function SystemUserEditDialog({ user, onClose }: SystemUserEditDialogProps) {
  ...
  const handleSubmit = async (formData: FormData) => {
    startTransition(async () => {
      try {
        const result = await saveSystemUser(formData)
        setState(result)
            ...
            <div className="space-y-2">
              ...
              <div className="grid grid-cols-2 gap-2 rounded-md border p-4">
                {Object.entries(accessTranslations).map(([key, label]) => {
                  ...
                  if (user?.access) {
                    try {
                      const accessObj = typeof user.access === 'string' ? JSON.parse(user.access) : user.access
                      isChecked = accessObj[key] ?? false
                    } catch {
                      isChecked = false
                    }
                  }
                  ...
                  return (
                    <div key={key} className="flex items-center space-x-2">
                      <Checkbox id={`access_${key}`} name={`access_${key}`} defaultChecked={isChecked} />

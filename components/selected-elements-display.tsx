...
export function SelectedElementsDisplay({
      ...
      >
        ...
        <div className="flex-shrink-0">
          {blockType === HOMEPAGE_BLOCK_TYPES.BENEFITS && element.icon ? (
            ...
            <div className="w-8 h-8 flex items-center justify-center bg-gray-100 rounded relative group">
              {getIcon(element.icon, "w-5 h-5 text-gray-600")}
              {/* Отладочная информация */}
              {process.env.NODE_ENV === 'development' && (
                <div className="absolute -top-8 left-0 text-xs bg-black text-white px-1 rounded opacity-0 group-hover:opacity-100 transition-opacity">
                  Icon: {element.icon}
                </div>
              )}
            </div>
  ...
  return (
    <div className={cn("flex flex-col h-full", className)}>
      ...
      {/* Список элементов с фиксированной высотой и скроллом */}
      <div 

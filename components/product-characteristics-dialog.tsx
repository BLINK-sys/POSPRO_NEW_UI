...
function SortableCharacteristicItem({
  ...
  onTempUpdate: (id: number, field: "name" | "value", value: string) => void
  isPending: boolean
}) {
      ...
      {char.isEditing ? (
        <>
          <Input
            ...
            value={char.tempName ?? char.name}
            onChange={(e) => onTempUpdate(char.id, "name", e.target.value)}
            className="flex-1"
            disabled={isPending}
          />
          <Input
        ...
        <>
          <div className="flex-1 px-3 py-2 text-sm">
            {char.name || <span className="text-gray-400">Без названия</span>}
          </div>
          <div className="flex-1 px-3 py-2 text-sm">
...
export function ProductCharacteristicsDialog({ productId, onClose }: ProductCharacteristicsDialogProps) {
  ...
  const handleAdd = () => {
    const newOrder = characteristics.length > 0 ? Math.max(...characteristics.map((c) => c.order)) + 1 : 1
  ...
  const handleDelete = (id: number) => {
    ...
    startTransition(async () => {
      try {
        await deleteCharacteristic(productId, id)
        toast({ title: "Характеристика удалена" })
        await fetchChars()
      } catch (error) {
        toast({ variant: "destructive", title: "Ошибка", description: error instanceof Error ? error.message : "Неизвестная ошибка" })
      }
    })
  }
  ...
  const handleEdit = (id: number) => {
    setCharacteristics((prev) =>
              prev.map((c) => (c.id === id ? { ...c, isEditing: true, tempName: c.name, tempValue: c.value } : c)),
    )
  }
  ...
  const handleSaveEdit = (id: number) => {
    ...
    const name = char.tempName || char.name
    ...
    if (char.isNew) {
      ...
      startTransition(async () => {
        try {
          const result = await addCharacteristic(productId, name, value)
  ...
  const handleCancelEdit = (id: number) => {
    ...
    if (char?.isNew) {
      ...
      setCharacteristics((prev) =>
        prev.map((c) => (c.id === id ? { ...c, isEditing: false, tempName: undefined, tempValue: undefined } : c)),
      )
    }
  }

  const handleTempUpdate = (id: number, field: "name" | "value", value: string) => {
    setCharacteristics((prev) =>
      prev.map((c) => (c.id === id ? { ...c, [field === "name" ? "tempName" : "tempValue"]: value } : c)),
    )
  }
  ...
  const handleSaveAll = () => {
    startTransition(async () => {
      try {
        ...
        const characteristicIds = existingChars.map((c) => c.id)

        await reorderCharacteristics(productId, characteristicIds)

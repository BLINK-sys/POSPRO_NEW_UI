...
export async function saveClient(formData: FormData): Promise<ActionState> {
  try {
    ...
    // Обработка полей в зависимости от типа организации
    const organizationType = clientData.organization_type as string
    
    if (organizationType === 'individual') {
      clientData.full_name = clientData.full_name || ''
    } else if (organizationType === 'ip') {
      clientData.ip_name = clientData.ip_name || ''
      clientData.iin = clientData.iin || ''
    } else if (organizationType === 'too') {
      clientData.too_name = clientData.too_name || ''
      clientData.bin = clientData.bin || ''
    }
...
export async function saveSystemUser(formData: FormData): Promise<ActionState> {
  try {
    ...
    const accessKeys = ["dashboard", "users", "catalog", "orders", "clients", "settings", "brands", "statuses", "pages"]

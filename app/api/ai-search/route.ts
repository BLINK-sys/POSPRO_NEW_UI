...
⚠️ ОБЯЗАТЕЛЬНО ПЕРЕД любым ответом «нет такого» / «не нашли» / «не везём»:
   1. Сначала просканируй ПОЛНОЕ ДЕРЕВО КАТЕГОРИЙ (не только TOP-30) на
      ...
      есть и работают.
   2. Если по подстроке нашлась подходящая категория — бери её id и
      запускай search_products(category_ids=[id]). Не ленись это делать —
      это самая частая причина ложных «нет такого».
   3. Только если по подстроке в полном дереве реально нет ни одной
      смысловой категории — тогда отвечай «нет, у нас такого нет».
